    ...
    foreach (QObject *obj, list) {
        if (obj->metaObject()->className() == QStringLiteral("QWidget")){
            ...
            foreach (QObject *obj1, list2){
                ...
                }else if(obj1->inherits("QComboBox")){
                    QComboBox* c=qobject_cast<QComboBox*>(obj1);
                    objectNameList.push_back(c->objectName());
                    temp.insert(c->objectName(),c);
                }else if(obj1->inherits("QCheckBox")){
                    QCheckBox* c=qobject_cast<QCheckBox*>(obj1);
                    objectNameList.push_back(c->objectName());
                    temp.insert(c->objectName(),c);
                }
            }
...
//    model->setItem(0,0,new QStandardItem("hello world"));
    ...
    qDebug()<<"objectNameList size: "<<objectNameList;
    _mapDelegate=new MapDelegate(objectNameList,this);
